...
CREATE TABLE ADM_Vistas(
    id_vista int GENERATED BY DEFAULT AS IDENTITY (START WITH 1, INCREMENT BY 1) not null UNIQUE,
...
CREATE TABLE ADM_Funcionalidad(
    id_vis_acc int GENERATED BY DEFAULT AS IDENTITY (START WITH 1, INCREMENT BY 1) not null UNIQUE,
    id_vista int NOT NULL,
    id_accion int NOT NULL,
    FOREIGN KEY (id_vista) REFERENCES ADM_Vistas(id_vista),
    FOREIGN KEY (id_accion) REFERENCES ADM_Acciones(id_accion));


CREATE TABLE ADM_ER_rol_funcionalidad(
    id_rol int NOT NULL,
    id_vis_acc int NOT NULL,
    FOREIGN KEY (id_rol) REFERENCES ADM_Roles(id_rol),
    FOREIGN KEY (id_vis_acc) REFERENCES ADM_Funcionalidad(id_vis_acc));
